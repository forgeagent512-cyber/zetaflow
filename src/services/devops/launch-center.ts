...
export class LaunchCenter {
  async runProductionChecklist(orgId: string): Promise<LaunchReport> {
  ...
  async runHealthChecks(orgId: string): Promise<CheckResult[]> {
  ...
  async runSecurityCheck(orgId: string): Promise<CheckResult[]> {
  ...
  async runSEOReadiness(orgId: string): Promise<CheckResult[]> {
    const supabase = getSupabase();
    const results: CheckResult[] = [];
  ...
  private async checkDeploymentConfig(orgId: string): Promise<CheckResult> {
    const supabase = getSupabase();
  ...
  private async checkLicenseStatus(orgId: string): Promise<CheckResult> {
    const supabase = getSupabase();
    const { data: license } = await supabase.from('licenses').select('*').eq('organization_id', orgId).single();
    if (!license) return { name: 'license_status', status: 'fail', message: 'No license found' };
  ...
  private async checkBillingSetup(orgId: string): Promise<CheckResult> {
  ...
  private async checkMonitoringConfig(orgId: string): Promise<CheckResult> {
    const supabase = getSupabase();
  ...
  private async checkBackupConfig(orgId: string): Promise<CheckResult> {
    const supabase = getSupabase();
  ...
  async generateLaunchReport(orgId: string): Promise<LaunchReport> {
    return this.runProductionChecklist(orgId);
  }

  async isReadyForLaunch(orgId: string): Promise<{ ready: boolean; report: LaunchReport }> {
