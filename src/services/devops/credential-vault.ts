...
export class CredentialVault {
  async storeCredential(orgId: string, name: string, provider: string, value: string): Promise<CredentialRecord> {
  ...
  async getCredential(id: string): Promise<{ record: CredentialRecord; value: string }> {
    const supabase = getSupabase();
  ...
  async getCredentials(orgId: string): Promise<CredentialRecord[]> {
    const supabase = getSupabase();
  ...
  async rotateCredential(id: string, newValue: string): Promise<CredentialRecord> {
    const supabase = getSupabase();
  ...
  private mapRow(row: any): CredentialRecord {
    return {
      id: row.id,
