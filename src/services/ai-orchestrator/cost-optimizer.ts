import type { CostEstimate, QualityTier, RouterRequest, RouterResponse } from './ai-orchestrator.types.js';
...
const TIER_COST_MULTIPLIERS: Record<QualityTier, number> = {
  economy: 1,
  balanced: 2.5,
  premium: 10,
  enterprise: 50
...
function getModelCostEntry(model: string): { inputCostPer1K: number; outputCostPer1K: number } {
...
export class CostOptimizer {
  ...
  async trackUsage(request: RouterRequest, response: RouterResponse): Promise<void> {
    const { error } = await this.supabase.from('ai_usage_logs').insert({
      organization_id: (request as unknown as Record<string, unknown>).orgId as string ?? 'unknown',
  ...
  estimateCost(model: string, prompt: string, tier: QualityTier): CostEstimate {
  ...
  async getUsageStats(orgId: string, period: 'day' | 'week' | 'month'): Promise<any> {
    const now = new Date();
    let startDate: Date;
    ...
    for (const row of data) {
      ...
      const taskType = (record.task_type as string) ?? 'unknown';
  ...
  async getCostReport(filters: any): Promise<any> {
