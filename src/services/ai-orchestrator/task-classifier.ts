import type { AiTaskType, ClassifiedTask } from './ai-orchestrator.types.js';

interface ClassifyPattern {
  taskType: AiTaskType;
  patterns: RegExp[];
  weight: number;
}

const CLASSIFY_PATTERNS: ClassifyPattern[] = [
  {
    taskType: 'coding',
    weight: 1.0,
    patterns: [
...
export class TaskClassifier {
  private cache = new Map<string, ClassifiedTask>();

  classify(prompt: string, context?: string): ClassifiedTask {
    ...
    let bestMatch: { taskType: AiTaskType; score: number; reasoning: string } | null = null;
