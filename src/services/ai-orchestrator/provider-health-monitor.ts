import type { ProviderHealthStatus } from './ai-orchestrator.types.js';
...
export class ProviderHealthMonitor {
  ...
  private healthCache: Map<string, { status: ProviderHealthStatus; timestamp: number }> = new Map();
  ...
  async getProviderStatus(provider: string): Promise<ProviderHealthStatus> {
    const now = Date.now();
    ...
    try {
      const providerInstance: AIProvider = ProviderFactory.create(provider as any);
  ...
  async autoDisableUnhealthy(): Promise<void> {
  ...
  async getModelStatus(model: string): Promise<any> {
  ...
  private async getProviderModels(provider: string): Promise<Array<{ model: string; status: string; latency: number }>> {
    try {
      const providerInstance: AIProvider = ProviderFactory.create(provider as any);
  ...
  private getFailureStats(provider: string): { totalFailures: number; consecutiveFailures: number; lastFailure: number } {
    let totalFailures = 0;
  ...
  private clearFailures(provider: string): void {
  ...
  private failureKey(provider: string, model: string): string {
