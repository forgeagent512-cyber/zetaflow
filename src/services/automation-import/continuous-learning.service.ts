...
export class ContinuousLearningService {
  ...
  private readonly indexer: TemplateIndexerService;
  private readonly templateSearch: TemplateSearchService;
  private readonly analyzerService: WorkflowAnalyzerService;
  private readonly enricher: TemplateEnricherService;
  private enrichedCache = new Map<string, unknown>();

  constructor(
    private readonly templateLibraryRoot: string = path.resolve(process.cwd(), 'template-library', 'source')
  ) {
    ...
    this.templateSearch = new TemplateSearchService(
      new InMemoryWorkflowGeneratorRepository(),
      this.analyzerService
    );
    this.enricher = new TemplateEnricherService();
  }

  async scanForChanges(): Promise<LearningSessionReport> {
    ...
    if (newFiles > 0 || updatedFiles > 0) {
      try {
        ...
        for (const filePath of currentHashes.keys()) {
          if (this.knownFiles.get(filePath) === currentHashes.get(filePath)) continue;
          try {
  ...
  async detectNewFiles(): Promise<FileChangeEvent[]> {
  ...
  async analyzeSingleTemplate(filePath: string): Promise<Record<string, unknown> | null> {
    try {
      const content = await readFile(filePath, 'utf8');
  ...
  getStats(): { totalTracked: number; lastScan: string | null } {
    return {
  ...
  private async findAllJsonFiles(root: string): Promise<string[]> {
...
export async function createContinuousLearningService(): Promise<ContinuousLearningService> {
