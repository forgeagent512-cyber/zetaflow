import type { AEOContent } from './seo.types.js';
...
export class AEOEngine {
  ...
  async generateFAQs(orgId: string, topic: string): Promise<AEOContent[]> {
...
[{"question": "string", "answer": "string", "keywords": ["string"], "schemaMarkupType": "FAQPage"}]`;
  ...
  async generateAnswerBlock(question: string): Promise<AEOContent> {
...
Return JSON:
{
  ...
  async generateHowToContent(topic: string, steps: string[]): Promise<AEOContent> {
...
Return JSON:
{
  ...
  "estimatedTime": "string"
    ...
    try {
      ...
      return {
        ...
        schemaMarkup: {
          '@type': 'HowTo',
          step: parsed.steps?.map((s: any, i: number) => ({
  ...
  async generateComparisonPage(items: Array<{ name: string; features: string[] }>): Promise<AEOContent> {
...
Return JSON:
{
  ...
  async generateGlossary(terms: Array<{ term: string; definition: string }>): Promise<AEOContent[]> {
    const entries: AEOContent[] = [];
...
{"answer": "string", "keywords": ["string"], "relatedTerms": ["string"]}`;
  ...
  async optimizeForFeaturedSnippets(content: string): Promise<string> {
