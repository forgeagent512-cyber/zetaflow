import type { AgentGeneratorInputDto, AgentGenerationOutputDto } from './agent-generator.dto.js';
import type { AgentGeneratorProvider } from './agent-generator.types.js';

export class GeminiAgentGenerator implements AgentGeneratorProvider {
  constructor(private readonly options: { apiKey: string; model?: string; endpoint?: string }) {}

  async generate(input: AgentGeneratorInputDto): Promise<AgentGenerationOutputDto> {
    ...
    return {
      ...
      agents: Array.isArray(parsed.agents) ? parsed.agents.map((agent: Record<string, unknown>) => ({
        ...
        goals: Array.isArray(agent.goals) ? agent.goals.filter((goal): goal is string => typeof goal === 'string') : ['Execute assigned workflow tasks'],
        tools: Array.isArray(agent.tools) ? agent.tools.filter((tool): tool is string => typeof tool === 'string') : (input.required_tools ?? ['workflow_engine', 'crm_api']),
        permissions: Array.isArray(agent.permissions) ? agent.permissions.filter((permission): permission is string => typeof permission === 'string') : ['read:customer-data', 'write:task-state'],
        memory: typeof agent.memory === 'string' ? agent.memory : 'short_term',
        llm: typeof agent.llm === 'string' ? agent.llm : 'gemini',
        trigger_events: Array.isArray(agent.trigger_events) ? agent.trigger_events.filter((event): event is string => typeof event === 'string') : ['new_request'],
        required_workflows: Array.isArray(agent.required_workflows) ? agent.required_workflows.filter((workflow): workflow is string => typeof workflow === 'string') : input.required_workflows,
        required_integrations: Array.isArray(agent.required_integrations) ? agent.required_integrations.filter((integration): integration is string => typeof integration === 'string') : input.required_integrations,
        personality: agent.personality && typeof agent.personality === 'object' ? agent.personality as { tone: string; style: string; behavior_traits: string[] } : {
          tone: 'professional',
          style: 'direct',
          behavior_traits: ['proactive', 'courteous', 'detail-oriented']
        },
        communication_rules: agent.communication_rules && typeof agent.communication_rules === 'object' ? agent.communication_rules as Record<string, unknown> : {
          sequential_execution: true,
          fallback_agent: 'operations',
          retry_strategy: 'exponential_backoff',
          timeout_strategy: 'escalate'
        },
        error_handling: agent.error_handling && typeof agent.error_handling === 'object' ? agent.error_handling as { strategy: 'retry' | 'fallback' | 'escalate' | 'ignore'; max_retries: number; fallback_action: string } : {
          strategy: 'retry',
          max_retries: 3,
          fallback_action: 'escalate'
        },
        retry_logic: agent.retry_logic && typeof agent.retry_logic === 'object' ? agent.retry_logic as { max_attempts: number; backoff_strategy: 'exponential' | 'linear' | 'fixed'; delay_ms: number } : {
          max_attempts: 3,
          backoff_strategy: 'exponential',
          delay_ms: 1000
        }
  ...
  private buildFallbackResponse(input: AgentGeneratorInputDto): AgentGenerationOutputDto {
    return {
      employee_name: input.employee_name,
      agents: [
        {
