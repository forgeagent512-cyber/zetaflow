...
import type { TemplateSearchInputDto, TemplateSearchResponseDto, TemplateMatchDto } from './template-search.dto.js';
import type { WorkflowGeneratorRepository } from './workflow-generator.types.js';
...
export interface SupabaseTemplateRecord {
  id: string;
...
export class TemplateSearchService {
  private templateIndex: SupabaseTemplateRecord[] = [];
  ...
  constructor(
    ...
    private readonly semanticSearch: SemanticSearchService = new SemanticSearchService({ embed: async (text: string) => this.hashEmbed(text) }),
    private readonly indexerService: TemplateIndexerService = new TemplateIndexerService()
  ) {}

  async search(input: TemplateSearchInputDto): Promise<TemplateSearchResponseDto> {
    const templates = await this.loadTemplates();
    const scored = await this.scoreTemplates(input, templates);

    const matches: TemplateMatchDto[] = scored.map(s => ({
  ...
  async loadTemplates(): Promise<SupabaseTemplateRecord[]> {
    if (this.templateIndex.length > 0) return this.templateIndex;
  ...
  private async scoreTemplates(
    input: TemplateSearchInputDto,
    templates: SupabaseTemplateRecord[]
  ): Promise<Array<{ template: SupabaseTemplateRecord; score: number }>> {
    const queryText = [
  ...
  private buildResponse(matches: TemplateMatchDto[], input: TemplateSearchInputDto): TemplateSearchResponseDto {
