...
router.post('/knowledge-graph', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = req.organizationId!;
...
router.post('/entity-pages', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = req.organizationId!;
    const { entities } = req.body;
    if (!entities || !Array.isArray(entities)) {
...
router.post('/topic-clusters', authenticate, async (req: Request, res: Response) => {
  try {
    const orgId = req.organizationId!;
