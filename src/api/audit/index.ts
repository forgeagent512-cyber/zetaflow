...
router.get('/logs', authenticate, requireRole('Super Admin', 'Organization Admin'), async (req: Request, res: Response) => {
  try {
...
router.get('/logs/admin', authenticate, requireRole('Super Admin'), async (_req: Request, res: Response) => {
  try {
    const supabase = getSupabase();
