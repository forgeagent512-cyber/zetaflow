...
router.get('/:id/usage', authenticate, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase.from('api_key_usage').select('*').eq('api_key_id', id).eq('organization_id', orgId).gte('timestamp', new Date(Date.now() - parsePeriod(period)).toISOString()).order('timestamp', { ascending: false });
        if (error)
            throw new Error(error.message);
        res.json({ success: true, data: data ?? [] });
    }
