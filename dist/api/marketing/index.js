...
router.post('/video-script', authenticate, async (req, res) => {
    try {
        ...
        switch (platform) {
            ...
            case 'demo':
                if (!product) {
                    res.status(400).json({ success: false, message: 'Product data required for demo script' });
                    return;
                }
...
router.post('/keywords', authenticate, async (req, res) => {
    try {
        ...
        switch (action) {
            case 'discover':
                if (!seed) {
                    res.status(400).json({ success: false, message: 'Seed keyword is required' });
                    return;
                }
...
router.post('/competitors', authenticate, async (req, res) => {
    try {
        ...
        switch (action) {
            ...
            case 'track':
                if (!competitor) {
                    res.status(400).json({ success: false, message: 'Competitor data is required' });
                    return;
                }
            ...
            case 'insights':
                if (!competitorId) {
                    res.status(400).json({ success: false, message: 'Competitor ID is required' });
                    return;
                }
            ...
            case 'compare':
                if (!pageUrl) {
                    res.status(400).json({ success: false, message: 'Page URL is required' });
                    return;
                }
