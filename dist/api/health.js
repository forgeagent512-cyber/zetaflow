...
export async function aiFactoryHealthHandler(req, res) {
    try {
...
export async function promptEngineHealthHandler(req, res) {
    const health = await checkModuleHealth('prompt-engine');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function workflowGeneratorHealthHandler(req, res) {
    const health = await checkModuleHealth('workflow-generator');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function businessAnalyzerHealthHandler(req, res) {
    const health = await checkModuleHealth('business-analyzer');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function employeeGeneratorHealthHandler(req, res) {
    const health = await checkModuleHealth('employee-generator');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function agentGeneratorHealthHandler(req, res) {
    const health = await checkModuleHealth('agent-generator');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function workflowAnalyzerHealthHandler(req, res) {
    const health = await checkModuleHealth('workflow-analyzer');
    return res.json({ success: health.status === 'ok', data: health });
}
export async function templateSearchHealthHandler(req, res) {
