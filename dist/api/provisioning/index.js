...
router.post('/provision', authenticate, requireRole('Super Admin', 'Organization Admin'), async (req, res) => {
    try {
...
router.get('/status/:orgId', authenticate, async (req, res) => {
    try {
        const status = await provisioningEngine.getProvisioningStatus(req.params.orgId);
