...
router.post('/checklist', authenticate, async (req, res) => {
    try {
        ...
        try {
            ...
            if (saved) {
                for (const item of items) {
                    const savedItem = saved.find((s) => s.item_id === item.id);
                    if (savedItem)
...
router.post('/health-check', authenticate, async (req, res) => {
    try {
        const supabase = getSupabase();
        const orgId = req.organizationId;
        const checks = {};
...
router.post('/generate-report', authenticate, async (req, res) => {
    try {
        ...
        let reportData = { organizationId: orgId, generatedAt: new Date().toISOString(), status: 'unknown', sections: [] };
        if (checklist) {
            const completedItems = (checklist.items ?? []).filter((i) => i.status);
        ...
        if (healthCheck) {
            const allPass = Object.values(healthCheck.checks ?? {}).every((c) => c.status === 'pass');
            reportData.sections.push({ name: 'Health Check', status: allPass ? 'All systems pass' : 'Issues detected', checks: healthCheck.checks });
        }
        const allHealthy = reportData.sections.every((s) => !s.status || s.status === 'All systems pass' || (s.completionRate && parseInt(s.completionRate) >= 80));
        reportData.status = allHealthy ? 'ready' : 'needs-attention';
        reportData.recommendations = [];
        if (!allHealthy)
            reportData.recommendations.push('Complete remaining checklist items and resolve health check issues before launch.');
...
router.get('/readiness/:orgId', authenticate, async (req, res) => {
    try {
        const supabase = getSupabase();
        ...
        if (org.status === 'active')
            score += 30;
        if (readiness.domainConfigured)
            score += 30;
        if (readiness.hasDeployments)
            score += 40;
        readiness.score = score;
        if (score < 60)
            readiness.recommendations.push('Configure your custom domain and create your first deployment.');
        else if (score < 100)
            readiness.recommendations.push('Complete remaining setup for full launch readiness.');
        else
            readiness.recommendations.push('Organization is ready for launch.');
