...
router.post('/generate-meta', authenticate, async (req, res) => {
    try {
...
router.post('/generate-schema', authenticate, async (req, res) => {
    try {
        ...
        const method = `generate${type}`;
        if (typeof schemaGenerator[method] === 'function') {
            const schema = await schemaGenerator[method](data);
...
router.post('/redirects', authenticate, async (req, res) => {
    try {
        const supabase = getSupabase();
        const orgId = req.organizationId;
...
router.get('/broken-links', authenticate, async (req, res) => {
    try {
        ...
        const brokenLinks = [];
        if (pages) {
            const urls = pages.map((p) => p.url).filter(Boolean).slice(0, 20);
