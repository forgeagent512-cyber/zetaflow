...
export function validateEmployeeGeneratorInput(input) {
    ...
    const requirements = Array.isArray(input.requirements) && input.requirements.every((item) => typeof item === 'string' && item.trim())
        ? input.requirements.filter((item) => typeof item === 'string').map((item) => item.trim())
        : [];
...
export function validateGeneratedEmployee(employee) {
