...
export class TemplateIndexerService {
    ...
    async searchSimilar(businessPurpose, industry, topK = 5) {
    ...
    toAutomationTemplateRow(record) {
        return {
    ...
    toTemplateVersionRow(templateId, workflowJson) {
        return {
            template_id: templateId,
            version: '1.0',
    ...
    toWorkflowAnalysisRow(templateId, analysis) {
        return {
            template_id: templateId,
            analysis,
            metadata: {
    ...
    toWorkflowEmbeddingRow(templateId, embeddingVector) {
        return {
            template_id: templateId,
    ...
    async analyzeWorkflow(workflow, workflowId) {
    ...
    fallbackAnalysis(workflow, workflowId) {
    ...
    inferIndustry(metadata, analysis) {
        ...
        if (combined.includes('real estate'))
            return 'Real Estate';
        if (combined.includes('lead') || combined.includes('crm') || combined.includes('sales'))
            return 'Sales';
        if (combined.includes('support'))
            return 'Customer Support';
        if (combined.includes('finance') || combined.includes('account'))
            return 'Finance';
        if (combined.includes('hr') || combined.includes('recruit'))
            return 'HR';
        return 'Operations';
    }
    inferDepartment(industry, category) {
        const deptMap = {
    ...
    estimateBusinessValue(automationScore) {
