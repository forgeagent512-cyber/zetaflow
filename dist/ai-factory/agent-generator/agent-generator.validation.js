...
export function validateAgentGeneratorInput(input) {
    ...
    const workflows = Array.isArray(input.required_workflows) ? input.required_workflows.filter((item) => typeof item === 'string' && item.trim().length > 0) : [];
    const integrations = Array.isArray(input.required_integrations) ? input.required_integrations.filter((item) => typeof item === 'string' && item.trim().length > 0) : [];
    const tools = Array.isArray(input.required_tools) ? input.required_tools.filter((item) => typeof item === 'string' && item.trim().length > 0) : [];
