...
export class ImportExportService {
    async exportEntity(orgId, entityType, ids) {
    ...
    async processExport(jobId, orgId, entityType, ids) {
        const supabase = getSupabase();
        try {
    ...
    async importEntity(orgId, entityType, data) {
        if (!ALLOWED_ENTITIES.includes(entityType)) {
    ...
    async processImport(jobId, orgId, entityType, records) {
        const supabase = getSupabase();
        try {
            let imported = 0;
    ...
    async processExportAll(jobId, orgId) {
        const supabase = getSupabase();
        try {
            const exportData = {};
    ...
    async processImportAll(jobId, orgId, data) {
        const supabase = getSupabase();
        try {
            let totalImported = 0;
