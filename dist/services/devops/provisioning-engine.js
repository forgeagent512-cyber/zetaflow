...
export class ProvisioningEngine {
    ...
    async processProvisioning(id, request) {
        const supabase = getSupabase();
        try {
    ...
    async updateStep(id, stepIndex, status) {
        const supabase = getSupabase();
    ...
    async generateAIWorkforce(orgId, industry) {
        const supabase = getSupabase();
        ...
        for (const emp of defaultEmployees) {
            ...
            if (error)
                throw new Error(`Failed to create AI employee '${emp.name}': ${error.message}`);
        }
    }
    async generateAPIKeys(orgId) {
        const supabase = getSupabase();
        const keys = [];
