...
export class WhiteLabelEngine {
    ...
    async applyBranding(orgId, htmlContent) {
        const settings = await this.getSettings(orgId);
        if (!settings)
            return htmlContent;
        let result = htmlContent;
        if (settings.logoUrl)
            result = result.replace(/{{LOGO_URL}}/g, settings.logoUrl);
        if (settings.primaryColor)
            result = result.replace(/{{PRIMARY_COLOR}}/g, settings.primaryColor);
        if (settings.faviconUrl)
            result = result.replace(/{{FAVICON_URL}}/g, settings.faviconUrl);
        if (settings.customCss)
            result = result.replace('</head>', `<style>${settings.customCss}</style></head>`);
        if (settings.customJs)
            result = result.replace('</body>', `<script>${settings.customJs}</script></body>`);
        if (settings.footerText)
            result = result.replace(/{{FOOTER_TEXT}}/g, settings.footerText);
        return result;
    }
    ...
    async verifyCustomDomain(orgId) {
        const settings = await this.getSettings(orgId);
