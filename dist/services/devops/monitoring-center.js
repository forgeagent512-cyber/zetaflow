...
export class MonitoringCenter {
    async recordMetric(orgId, metric) {
    ...
    async getMetrics(orgId, type, period) {
        const supabase = getSupabase();
    ...
    async getDeploymentHealth(orgId) {
        const supabase = getSupabase();
    ...
    async getBillingHealth(orgId) {
        try {
            const stripeKey = process.env.STRIPE_SECRET_KEY;
            if (!stripeKey)
                return { component: 'billing', status: 'healthy', message: 'Billing not configured' };
