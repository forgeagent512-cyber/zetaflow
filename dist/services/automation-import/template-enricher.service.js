...
export class TemplateEnricherService {
    ...
    async detectIndustry(text, fallback) {
        ...
        if (/(property|house|apartment|rent|lease|mortgage|realtor|listing)/i.test(text))
            return 'Real Estate';
        if (/(patient|clinic|doctor|hospital|medical|health)/i.test(text))
            return 'Healthcare';
        if (/(lawyer|court|legal|attorney|lawsuit)/i.test(text))
            return 'Legal';
        if (/(restaurant|menu|order|food|kitchen|dining)/i.test(text))
            return 'Restaurant';
        if (/(course|student|teacher|classroom|school|education|learning)/i.test(text))
            return 'Education';
        if (/(invoice|accounting|bookkeep|tax|audit|finance)/i.test(text))
            return 'Accounting';
        if (/(ship|delivery|warehouse|inventory|logistics|transport)/i.test(text))
            return 'Logistics';
        if (/(shop|product|cart|checkout|store|ecommerce|e-commerce)/i.test(text))
            return 'E-commerce';
        return fallback || 'General Business';
    }
    async detectCategory(text, fallback) {
        if (/(lead|qualify|score|capture)/i.test(text))
            return 'Lead Qualification';
        if (/(support|ticket|help|faq|question)/i.test(text))
            return 'Customer Support';
        if (/(booking|appointment|schedule|calendar)/i.test(text))
            return 'Appointment Booking';
        if (/(email|mail|newsletter|campaign)/i.test(text))
            return 'Email Automation';
        if (/(crm|pipeline|deal|contact|account)/i.test(text))
            return 'CRM';
        if (/(invoice|payment|billing|receipt|refund)/i.test(text))
            return 'Finance';
        if (/(social|post|content|blog|media)/i.test(text))
            return 'Marketing';
        if (/(chat|conversation|messag|whatsapp|telegram)/i.test(text))
            return 'Messaging';
        if (/(hr|employee|hiring|onboard|payroll)/i.test(text))
            return 'HR';
        if (/(ai|intelligent|gpt|llm|model|predict)/i.test(text))
            return 'AI Automation';
        return fallback || 'Operations';
    }
    extractTagsAndKeywords(text, existingTags, existingKeywords) {
