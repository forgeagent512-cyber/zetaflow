...
Return only the translated content:`;
    ...
    async generateLocalizedSEO(orgId, locale, source) {
...
Return JSON:
{
  ...
  "localeSpecificNotes": "string"
    ...
    async generateLocalizedSchema(schema, locale) {
        ...
        if (localized.step && Array.isArray(localized.step)) {
            for (const step of localized.step) {
                if (step.name)
                    step.name = await this.translateContent(step.name, locale);
                if (step.text)
                    step.text = await this.translateContent(step.text, locale);
            }
        }
        return localized;
    }
    async generateLocalizedURLs(orgId, baseUrl, locales) {
        const urls = [];
