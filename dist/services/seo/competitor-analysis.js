...
Return JSON:
{
  ...
  "opportunities": ["string"]
    ...
    async trackCompetitor(orgId, competitor) {
    ...
    async getCompetitorInsights(orgId, competitorId) {
        const supabase = this.getSupabase();
    ...
    async compareWithCompetitors(orgId, pageUrl) {
        const supabase = this.getSupabase();
...
Return JSON:
{
  ...
  "marketPosition": "string (leader, challenger, niche player, etc.)"
    ...
    async generateRecommendations(orgId, competitorId) {
        const supabase = this.getSupabase();
        const { data: competitor, error } = await supabase
            .from('competitors')
            .select('*')
