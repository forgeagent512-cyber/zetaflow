...
export class SEOAnalyzer {
    ...
    async analyzePage(url, content) {
        ...
        const score = Math.round((titleAnalysis.score +
    ...
    checkReadability(content) {
        ...
        if (avgSentenceLength > 25)
            issues.push(`Average sentence length ${Math.round(avgSentenceLength)} is too high, aim for under 20 words`);
        const words = content.split(/\s+/).filter(Boolean);
        const avgWordLength = words.reduce((sum, w) => sum + w.length, 0) / words.length;
        if (avgWordLength > 6)
            issues.push(`Average word length ${avgWordLength.toFixed(1)} is high, use simpler vocabulary`);
        const paragraphs = content.split(/\n\s*\n/).filter(p => p.trim().length > 0);
        if (paragraphs.some(p => p.split(/\s+/).filter(Boolean).length > 300))
            issues.push('Some paragraphs exceed 300 words, break them up');
...
Content: ${content.slice(0, 1000)}`;
    ...
    async analyzeHeadings(content) {
        ...
        if (h1.length === 0 && !markdownHeadings.some(h => h.startsWith('# ')))
    ...
    async analyzeImages(content) {
        ...
        if (count > 0 && withAlt < count)
            issues.push(`${count - withAlt} images missing alt text`);
    ...
    async analyzePerformance(content) {
        ...
        if (scriptCount > 10)
            issues.push(`Page has ${scriptCount} scripts, consider reducing`);
        const imageCount = (content.match(/<img/gi) ?? []).length;
        if (imageCount > 10)
            issues.push(`Page has ${imageCount} images, consider lazy loading`);
        const cssCount = (content.match(/<link[^>]*rel=["']stylesheet["']/gi) ?? []).length;
        if (cssCount > 3)
            issues.push(`Page has ${cssCount} CSS files, consider combining`);
        const score = Math.max(0, 100 - issues.length * 33);
        return { score, issues };
    }
}
