...
test('rejects none algorithm tokens', () => {
    ...
    const token = jwt.sign({ userId: 'u1', organizationId: 'org1', role: 'Customer', email: 'a@example.com' }, 'ignored', { algorithm: 'none' });
    assert.throws(() => service.verifyToken(token), /Invalid or unsupported token/i);
});
test('rejects unexpected signing algorithms', () => {
    process.env.JWT_SECRET = 'test-secret';
    const service = new AuthService();
    const token = jwt.sign({ userId: 'u1', organizationId: 'org1', role: 'Customer', email: 'a@example.com' }, 'test-secret', { algorithm: 'HS384' });
    assert.throws(() => service.verifyToken(token), /Invalid or unsupported token/i);
});
